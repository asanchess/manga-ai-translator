...
def test_circular_bubble_fitting():
    ...
    assert words_count >= 12, f"Expected at least 12 words, got {words_count}"
    ...
    assert len(text_x_indices) > 0, "Text was not rendered onto the bubble!"
    ...
    # All text pixels must be within safe radius <= 85% of r (85% of 75 = 63.75)
    print(f"Max text pixel distance from center: {max_dist:.2f}px (Circle Radius: {r}px, 85% Safe Limit: {r * 0.85:.2f}px)")
    assert outside_pixels == 0, f"Found {outside_pixels} text pixels leaking outside circle radius {r}px!"
    assert max_dist <= r * 0.88, f"Text exceeded safe oval radius: max_dist={max_dist:.2f}px > {r * 0.88:.2f}px"
    print("  [PASS] Circular bubble boundary test: PASSED (Zero text bleed).")

def test_dark_bubble_contrast():
    ...
    assert white_pixels > 50, "Dark bubble failed to render bright white text with auto-contrast!"
    print("  [PASS] Dark bubble auto-contrast test: PASSED (White text on dark background).")

if __name__ == "__main__":
    print("==================================================")
    print("  RUNNING TYPESETTER LAYOUT UNIT TESTS")
    print("==================================================")
    test_circular_bubble_fitting()
    test_dark_bubble_contrast()
    print("==================================================")
    print("  ALL TYPESETTER LAYOUT TESTS PASSED!")
    print("==================================================")
