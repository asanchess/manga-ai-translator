...
def reprocess_all_chapter_531():
    ...
    print(f"Reprocessing {len(pages)} pages of Chapter 531 with FIXED OCR & CLEANER...")
    
    for idx, p in enumerate(pages):
        print(f"\n--- Processing [{idx+1}/{len(pages)}] {p} ---")
        orig_p = os.path.join(orig_dir, p)
        clean_p = os.path.join(clean_dir, p)
        trans_p = os.path.join(trans_dir, p)
        
        # 1. OCR with new Containment NMS
        clusters = extract_text_and_bubbles(orig_p, use_cache=False)
        print(f"  OCR Clusters ({len(clusters)}):")
        for i, c in enumerate(clusters):
            print(f"    [{i+1}] {c['box']} text='{c['text'][:35]}'")
        ...
        print(f"  ✓ {p} successfully reprocessed!")

    # Crop page 8 panels to verify fix
    p8_trans = os.path.join(trans_dir, "page_008.webp")
    if os.path.exists(p8_trans):
        img = cv2.imread(p8_trans)
        if img is not None:
            crop = img[5000:7000, :]
            cv2.imwrite(os.path.join(os.path.dirname(__file__), "eval_p8_fixed.png"), crop)
            print("Exported eval_p8_fixed.png!")

if __name__ == "__main__":
    reprocess_all_chapter_531()
